...
/**
 * Persistent index of discovered txt files. Hydrating this at startup avoids the
 * 2-minute cold-start cost of enumerating + parsing 10k files on every launch.
 *
 * Schema v2: paths + mtime. The actual SongMeta is rebuilt as
 * LazyLoadedFromFileSongMeta which lazy-parses voices on demand.
 */
public static class SongIndexCache
{
    ...
    public static List<CacheEntry> Load()
    {
        ...
        try
        {
            ...
            {
                Debug.Log($"SongIndexCache: stale or missing schema (got {cache?.version}, want {SchemaVersion}) — ignoring");
                return new List<CacheEntry>();
            }
    ...
    public static void Save(IReadOnlyCollection<CacheEntry> entries)
    {
        string path = GetCachePath();
    ...
    public static bool IsCacheEntryFresh(CacheEntry entry)
    {
        ...
        try
        {
            ...
            return fi.LastWriteTimeUtc.Ticks == entry.mtimeUtcTicks;
        }
        catch
        {
            return false;
